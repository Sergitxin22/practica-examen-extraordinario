...
public class Mision implements Serializable, Comparable<Mision> {
	...
	public String toString() {
		return "Mision " + nombre + " a " + destino + " (" + fecha + ", nave: " + 
				nave.getNombre() + ", personal: " + personal.size() + ", costeTotal: " + getCosteTotal() +"M$)";
	}
	...
	public double getCosteTotal() {
		double coste = 1.5; // en millones
		if (!nave.equals(null)) {
			coste += nave.getCoste();
		}
		
		for (Personal persona : personal) {
			coste += persona.getCoste();
		}
		return coste;  // TODO Cambiar este código - programado de manera que pueda usarse para la tarea 3B aunque no se haya resuelto la 2C
	}
	...
	// TAREA 2F: ordenar en función del nombre del destino
